...
#define FILL_IF_EXACTLY_ONE_RECUR(LOOP1, LOOP2, num_can_fill) \
    do { \
        LOOP1 { \
            if (num_can_fill == 1) { \
                LOOP2 { \
...
bool row(Sudoku *thiz, int r, int c, int n)
{
    FILL_IF_EXACTLY_ONE_RECUR(
        FOR_EACH_NUM(n) if(!thiz->row[r][n]),
        FOR_N(c),
        thiz->num_can_fill_row[r][n]
    );
}
bool col(Sudoku *thiz, int r, int c, int n)
{
    FILL_IF_EXACTLY_ONE_RECUR(
        FOR_EACH_NUM(n) if(!thiz->col[c][n]),
        FOR_N(r),
        thiz->num_can_fill_col[c][n]
    );
}
bool box(Sudoku *thiz, int r, int c, int n)
{
    int i = get_box(r, c);
    FILL_IF_EXACTLY_ONE_RECUR(
        FOR_EACH_NUM(n) if(!thiz->box[i][n]),
        FOR_R_C_IN_BOX(r, c, i),
        thiz->num_can_fill_box[i][n]
    );
}

void sudoku_fill_recursion(Sudoku *thiz, int r, int c, int n)
{
    row(thiz, r, c, n);
    col(thiz, r, c, n);
    box(thiz, r, c, n);
...
void sudoku_fill(Sudoku *thiz, int r, int c, int n)
{
    ...
    sudoku_fill_recursion(thiz, r, c, n);
}
